...
- [ ] T001 Install dependencies from package.json: tsdav, typescript@5.8.3+, esbuild, vitest
- [ ] T002 [P] Configure TypeScript compiler in tsconfig.json with strict mode enabled
- [ ] T003 [P] Create src/ directory structure with subdirectories: caldav/, sync/, vault/, ui/
- [ ] T004 [P] Create base TypeScript interfaces in src/types.ts for Task, SyncMapping, CalDAVConfiguration, CalDAVTask, TaskStatus, VTODOStatus enums
- [ ] T005 [P] Update manifest.json with plugin metadata: id, name, version, minAppVersion, description, isDesktopOnly: false
- [ ] T006 Verify build process works: npm run build produces main.js
...
- [ ] T007 Create plugin lifecycle skeleton in src/main.ts with onload(), onunload(), loadSettings(), saveSettings() methods
- [ ] T008 [P] Implement settings persistence in src/settings.ts with CalDAVConfiguration interface and default values
- [ ] T009 [P] Create date parsing utilities in src/vault/taskParser.ts: parseTasksPluginDate(), toCalDAVDate(), parseCalDAVDate() functions
- [ ] T010 [P] Create content hash utility in src/sync/mapping.ts: hashTaskContent() function for change detection
- [ ] T011 [P] Create block reference ID generator in src/vault/blockRefManager.ts: generateTaskBlockId() using crypto.randomUUID()
- [ ] T012 Implement error types in src/caldav/errors.ts: CalDAVError, CalDAVAuthError, CalDAVNetworkError, CalDAVConflictError classes
- [ ] T013 Implement SyncMapping storage in src/sync/mapping.ts: loadMappings(), saveMappings(), getMappingByBlockId(), setMapping(), removeMapping() methods
...
- [ ] T014 [P] [US4] Create settings tab UI in src/ui/settingsTab.ts extending PluginSettingTab with connection section
- [ ] T015 [US4] Add server URL input field in src/ui/settingsTab.ts with HTTPS validation
- [ ] T016 [US4] Add username input field in src/ui/settingsTab.ts
- [ ] T017 [US4] Add password input field in src/ui/settingsTab.ts with password masking
- [ ] T018 [US4] Add calendar path input field in src/ui/settingsTab.ts with default value suggestion
- [ ] T019 [US4] Implement CalDAV client wrapper in src/caldav/client.ts: CalDAVClient class with connect(), disconnect(), testConnection() methods using tsdav
- [ ] T020 [US4] Add "Test Connection" button in src/ui/settingsTab.ts that calls CalDAVClient.testConnection() and displays result
- [ ] T021 [US4] Implement connection validation in src/caldav/client.ts with authentication error handling
- [ ] T022 [US4] Add success/error notifications in src/ui/settingsTab.ts using Obsidian Notice API
- [ ] T023 [US4] Register settings tab in src/main.ts using this.addSettingTab()
...
- [ ] T024 [P] [US5] Add sync settings section to src/ui/settingsTab.ts
- [ ] T025 [US5] Add sync interval input field in src/ui/settingsTab.ts with default 60 seconds and minimum 10 seconds validation
- [ ] T026 [US5] Add enable/disable auto-sync toggle in src/ui/settingsTab.ts with default enabled
- [ ] T027 [US5] Implement sync scheduler in src/sync/scheduler.ts: SyncScheduler class with start(), stop(), reset() methods
- [ ] T028 [US5] Register sync interval in src/main.ts using this.registerInterval() for automatic cleanup
- [ ] T029 [US5] Add manual sync command in src/main.ts using this.addCommand() with id: 'manual-sync'
- [ ] T030 [US5] Implement timer reset logic in src/sync/scheduler.ts when manual sync is triggered
- [ ] T031 [US5] Add sync status notification in src/ui/notifications.ts: showSyncStart(), showSyncSuccess(), showSyncError() functions
- [ ] T032 [US5] Implement modal error notification in src/ui/notifications.ts using Obsidian Modal API for automatic sync errors
